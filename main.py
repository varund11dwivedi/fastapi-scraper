...
def get_jobs():
    ...
    # 1. We Work Remotely
    try:
        wwr_response = requests.get("https://weworkremotely.com/remote-jobs", headers={"User-Agent": "Mozilla/5.0"})
        ...
        for job in sections:
            ...
            if link:
                ...
                company = job.find("span", class_="company").text.strip() if job.find("span", class_="company") else ""
                position = job.find("span", class_="title").text.strip() if job.find("span", class_="title") else ""
                jobs.append({
    ...
    # 2. Jobspresso
    try:
        jsp_response = requests.get("https://jobspresso.co/remote-developer-jobs/", headers={"User-Agent": "Mozilla/5.0"})
    ...
    # 3. SkipTheDrive
    try:
        std_response = requests.get("https://www.skipthedrive.com/remote-software-development-jobs/", headers={"User-Agent": "Mozilla/5.0"})
